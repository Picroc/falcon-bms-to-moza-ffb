...
fn main() {
    ...
    let socket: TcpListener  = connect_to_telemetry(socket_host, socket_port).expect("Can't create socket!");
...
fn main_loop(stream: &mut TcpStream, flight_data_file: MemoryFile<'static, FlightData>, intellivibe_data_file: MemoryFile<'static, IntellivibeData>) {
    ...
    loop {
        ...
        if intellivibe_data_file.exit_game {
            ...
            if intellivibe_data_file.paused || intellivibe_data_file.ejecting || intellivibe_data_file.end_flight {
                continue;
            }
